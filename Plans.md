...
| Task | Description | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 2.1 | [setup] Add `@modelcontextprotocol/sdk` dependency; scaffold `src/mcp-server.ts` as a stdio MCP server entrypoint with zero tools registered yet. `src/cli.ts` and its `bin`/`commander` dependency are left untouched here (the CLI keeps working) — actual retirement is task 2.7's job, once nothing still needs it. | `npm run build` succeeds; `node dist/mcp-server.js` starts and exits cleanly on stdin close; all pre-existing Phase 1 tests still pass unmodified. | Phase 1 | cc:完了 [244b346] |
| 2.2 | [tdd:required] `src/verify-detect.ts`: given a repo path, read `package.json`, detect `scripts.lint` / `scripts.typecheck` (or `type-check`) / `scripts.test`, return the present ones as `["npm run <name>", ...]` in that order. | Unit tests over fixture `package.json` variants: all three scripts present, partial, none present, malformed JSON. | 2.1 | cc:完了 [9d842d3] |
| 2.3 | [tdd:required] `src/repo-detect.ts`: resolve the target repo path — explicit input wins; else MCP `roots/list`; else `CLAUDE_PROJECT_DIR` env; else a clear validation error (never silently defaults to cwd). | Unit test each fallback branch independently with mocked inputs, plus the no-resolution error case. | 2.1 | cc:完了 [7d240cd] |
...
| 2.10 | [docs] `README.md` (English, default), `README.zh-Hant.md` (繁體中文), and `README.ja.md` (日本語) already exist as separate, cross-linked files (split done ahead of schedule, 2026-07-09). This task updates all three in lockstep once the plugin is real: replace the CLI install/usage instructions with the real plugin flow — skills-dir drop-in steps for Claude Code, best-effort steps for Codex, `/harness-run` usage, auto-detect behavior + override syntax, unchanged state/escalation-file behavior. | All three files reviewed against the actually-implemented plugin — zero remaining references to `npm link` / `harness run --task ...` CLI syntax, and the three files stay in sync with each other (no content drift between languages). | 2.9 | cc:TODO |
